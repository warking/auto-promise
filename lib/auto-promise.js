  ...
  return new Promise((resolve, reject) => {
    ...
    let dependencies = opkeys.reduce((out, item) => {
      if(typeof ops[item] === 'function') {
        params[item] = out[item] = extractArgs(ops[item]);
        if(~['callback', 'cb'].indexOf(last(params[item]))) {
          // Looks like a callback-style function, promisify it
          params[item] = out[item] = params[item].slice(0, -1);
          ops[item] = promisify(ops[item]);
        }
      } else if(Array.isArray(ops[item])) {
        out[item] = ops[item].slice(0, -1);
        // Assume last element is the value/function, redefine operation
        ops[item] = ops[item].slice(-1)[0];
        params[item] = 'results';
      } else {
        out[item] = [];
      }
    ...
    let resolver = () => {
      ...
      Object.keys(dependencies).forEach(opkey => {
        ...
        if(typeof value === 'function') {
          ...
          if(!deps.filter(d => !results[d]).length) {
