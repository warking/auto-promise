  ...
  return new Promise((resolve, reject) => {
    ...
    let dependencies = taskNames.reduce((out, taskName) => {
      ...
      if (typeof task === 'function') {
        ...
        let args = extractArgs(fn);
        tasks[taskName] = looksLikeCallback(args, true) ? promisify(fn, true) : fn;
      } else {
        out[taskName] = [];
      }
      return out;
...
function promisify(fn, reverse, coalesce) {
  return function() {
    let args = Array.prototype.slice.call(arguments);
    return new Promise((resolve, reject) => {
      args[reverse ? 'unshift' : 'push']((err, result) => {
        // args.unshift((err, result) => {
        if (err) {
          return reject(err);
        }
